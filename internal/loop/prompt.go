...
func GenerateAgentPrompt(contract PromptContract) string {
	...
	fmt.Fprintf(&b, "- Run relevant feedback loops before passing task.\n")
	fmt.Fprintf(&b, "- For named feedback commands, prefer `goralph feedback run <name>`; use raw configured commands only when no name exists.\n")
	fmt.Fprintf(&b, "- Ralph v1 will not run or enforce feedback commands after you exit.\n")
	fmt.Fprintf(&b, "- Report feedback command success or failure with progress entries and final task status.\n")
